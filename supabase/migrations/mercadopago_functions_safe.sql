-- SUPABASE MIGRATIONS - MERCADO PAGO FUNCTIONS (SAFE VERSION)
-- -----------------------------------------------------------
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'admin_mercado_pago_keys' 
        AND policyname = 'Admins can manage their own keys'
    ) THEN
...
DROP TRIGGER IF EXISTS trigger_update_admin_keys_updated_at ON admin_mercado_pago_keys;
...
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Admins can view their payment records'
    ) THEN
