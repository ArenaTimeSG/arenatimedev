-- Script para corrigir a tabela payments existente (versão 2)
...
-- Verificar se existe um tipo ENUM para payment_status
SELECT typname, enumlabel 
FROM pg_type t 
JOIN pg_enum e ON t.oid = e.enumtypid 
WHERE typname LIKE '%payment%' OR typname LIKE '%status%';

-- Remover constraints existentes que podem estar causando conflito
DO $$
BEGIN
    ...
    -- Remover constraint de currency se existir
    IF EXISTS (SELECT 1 FROM information_schema.check_constraints 
...
-- Adicionar colunas que faltam (sem constraints por enquanto)
DO $$
BEGIN
...
-- Agora vamos verificar se a coluna status já existe e qual é seu tipo
SELECT column_name, data_type, udt_name 
FROM information_schema.columns 
WHERE table_name = 'payments' AND column_name = 'status';

-- Se a coluna status não existir, criar como VARCHAR
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
